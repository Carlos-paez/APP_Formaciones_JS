document.addEventListener("DOMContentLoaded", function () {
  ...
  async function checkAlerts() {
    try {
      const response = await fetch("/api/alerts/check?_=" + Date.now(), {
        headers: { "Cache-Control": "no-cache" },
      });
  ...
  function showEquipmentAlert(event, customMessage) {
    ...
    showSystemNotification(
      "🔧 Equipos necesitan reconfiguración",
  ...
  console.log(
    "%c✅ Sistema de Gestión de Eventos Iniciado",
    "color: #2c5282; font-weight: bold; font-size: 16px;",
  );
  console.log(`📊 Eventos cargados: ${allEvents.length}`);
  console.log(
    `⏱️ Verificación de alertas: cada ${ALERT_CHECK_INTERVAL / 1000} segundos`,
  );
  console.log(
    `🔄 Actualización de estados: cada ${STATUS_UPDATE_INTERVAL / 1000} segundos`,
  );
});
